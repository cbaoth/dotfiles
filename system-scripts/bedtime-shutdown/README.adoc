...
** *An optional "Mount Cleanup" step can be enabled* _before the forced shutdown to help mitigate potential filesystem issues (especially for ntfs/exFAT). When all features are enabled, the script will perform the following steps:_
[arabic]
... Stopping automount units if systemd-automount is used to prevent new mounts during the shutdown process.
... Terminate processes using the filesystems to be unmounted, to ensure they can be safely unmounted.
... Sync the disks to flush all pending writes.
... Unmount the filesystems to prevent potential corruption.
... Final buffer flush with another sync before the forced shutdown.
...
The suggested timer shown above is set to start much earlier (10:00) than the actual bedtime start in the script configuration file (default: 21:30), but does not cover 24 hours. This is intentional, for the following reasons:
* In case you configure the "Bedtime Zone" to start earlier than before, this way there is usually no need to remember and update the timer configuration as well, since the timer already covers a wide time range (10:00 to 05:00).
** Note that the script itself will check the actual time and exit early if it's not within the "Bedtime Zone", so there is no risk of it shutting down your system at 10:00.
* In case your configuration accidentally covers the entire 24 hours, or the script has a bug, the time range between 05:00 and 10:00 will always be safe.
