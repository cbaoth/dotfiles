...
# shellcheck shell=bash disable=SC2148,SC1083
#
# ~/lib/functions.sh: Shell utility functions for interactive use.
#
# Shell-agnostic: compatible with both bash and zsh.
# Sourced by .bashrc and .zshrc. Requires commons.sh to be loaded first.
  ...
  sed 's_.*/__' "$tempfile" | LC_ALL=C sort | uniq -d | while IFS= read -r f; do
    ...
    if [[ $show_index -eq 1 ]]; then
      ...
      while IFS= read -r line; do
        ...
        printf "%02d: %s\n" "$line_num" "$line"
      done < <(grep -F "/$f" "$tempfile" | LC_ALL=C sort "${sort_args[@]}")
      continue
    else
...
__mount_mountpoints() {
  ...
  [[ -n "${ZSH_VERSION:-}" ]] && setopt local_options nonomatch no_err_exit
  for arg in "$@"; do
    if [[ -n "${ZSH_VERSION:-}" ]]; then
      expanded=(${~arg})  # zsh: force glob expansion of variable content
    else
      expanded=($arg)     # bash: word split + glob (no setopt nonomatch; literal on no match)
    fi
...
EOF
  ...
  while IFS= read -r line; do
    [[ -n "$line" ]] && lines+=("$line")
  done < <(pgrep -ai "${pattern}" 2>/dev/null)
  ...
  for line in "${lines[@]}"; do
    ...
    if [[ "${cmd_lower}" =~ '^[a-z]:\\' ]]; then
      ...
      exe_win="${cmd:0:${exe_end}}"          # original-case exe path (bash: 0-based)
      args="${cmd:${exe_end}}"               # remaining args (incl. leading space)

      drive="$(printf '%s' "${exe_win:0:1}" | tr '[:upper:]' '[:lower:]')"  # drive letter, lowercased
      win_path="${exe_win:3}"                # strip "X:\" prefix (3 chars)
...
ytp() {
  ...
  youtube-dl --format 'bestvideo[vcodec=vp9]+bestaudio[acodec=opus]
    ...
    | sed '$!N;s/\n/\n  out=/' \
    | aria2c -U "$UAGENT" --continue \
             --max-connection-per-server=4 --max-concurrent-downloads=4 \
             --deferred-input -i -
}
...
ytap() {
  ...
  youtube-dl --format 'bestaudio[acodec=opus]/bestaudio[acodec=vorbis]
                       /best[ext=webm]/best[ext=ogg]/best' \
             --ignore-errors \
             --extract-audio \
             --output '%(title)s [%(id)s].ogg' \
             --get-url --get-filename "${@}" \
    | sed '$!N;s/\n/\n  out=/' \
    | aria2c -U "$UAGENT" --continue \
             --max-connection-per-server=4 --max-concurrent-downloads=4 \
...
# {{{ = WSL ==================================================================
# cd using windows path "c:\.."
cdw () {
  local lpath="$(sed -r 's/\\/\//g;s/^([A-Z]):/\/mnt\/\L\1/' <<<$1)"
  cd "$lpath"
}
# }}} = WSL ==================================================================
