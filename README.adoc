...
|`link:.zsh.d/zshrc-freebsd.zsh[]` +
 ...
 `link:.zsh.d/zshrc-11001001_org.zsh[]`|{icon-shell} ZSH|OS, distribution, and/or host specific ZSH configs.
|`link:.zsh.d/aliases.zsh[]`|{icon-shell} SH|Common aliases
|`link:.zsh.d/aliases-freebsd.zsh[]` +
 `link:.zsh.d/aliases-linux.zsh[]`|{icon-shell} SH|OS, distribution, and/or host specific aliases.
|`link:.zsh.d/functions.zsh[]`|{icon-shell} SH|Common shell functions.
|`link:.zsh.d/functions-freebsd.zsh[]`|{icon-shell} SH|OS, distribution, and/or host specific shell aliases.
|`link:.bash.d/bashrc[]`|{icon-shell} https://en.wikipedia.org/wiki/Bash_(Unix_shell)[Bash]|Common Bash config _(rarely maintained)_.
|`link:.profile[]`|{icon-shell}{icon-xwin}|Currently only loaded from inside `.xsession` to prepare the environment (variables) without a login shell. It's basically a copy of `.zshenv` though the latter can contain `zsh` specific stuff.
...
|===
|File|Description
|`link:link.sh[]`|Link all dotfiles (except local scripts and documentation) to the user's home directory. Missing directories are created in the process. Existing files that would be overwritten are moved to a backup location.
|`link:permissions.sh[]`|Update dotfile repo file permissions.
|`link:fix-modelines.py[]`|Normalize and deduplicate editor modelines (Emacs, Vim, VS Code, ShellCheck) across shell scripts.
