...
* `C-x, r`: `"^Xr" history-incremental-search-backward` &rarr; Incrementally search backward through command history.
* `C-r`: `"^R" zaw-history` &rarr; Search/filter command history with `zaw` fuzzy finder.
* `C-x a`: `"^Xa" _expand_alias` &rarr; Expand aliases in the current command line.
