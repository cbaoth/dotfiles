...
- Underscore prefix (`_helper`) is acceptable in sourced files to distinguish internal helpers from user-facing functions, but not required in standalone scripts.
  - Always consider to `unset`/`unfunction` temporary functions and variables when they are no longer needed.
- Avoid shadowing env vars: never use `PATH`, `PWD`, `HOME`, `USER`, `CMD`, `LOG`, etc. as local names.

## Variable Declarations

- Always use `declare` or `local` for variable declarations (no implicit globals); avoid the use of `typeset` unless specifically required for compatibility.
  - Use `local -r` / `declare -r` for readonly
  - Use `local -i` / `declare -i` for integers
  - Use `local -a` / `declare -a` for arrays
  - Use `local -A` / `declare -A` for associative arrays
- Separate declaration from assignment for command substitution: `local result; result="$(cmd)"; local -r result`
...
- Use `while` / `case` / `shift` pattern with `usage()` heredoc and `--` separator.
