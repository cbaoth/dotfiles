...
*📖 Full Documentation:* See link:README-bedtime-shutdown.adoc[README-bedtime-shutdown.adoc] for detailed installation, configuration, and usage instructions.

*Quick Start:*

[source,shell]
----
# Test with dry-run (no sudo needed!)
cp etc/bedtime-shutdown.conf /tmp/bedtime-shutdown.conf
editor /tmp/bedtime-shutdown.conf
./bedtime-shutdown.sh --config /tmp/bedtime-shutdown.conf --dry-run -v
...
# ... create systemd service and timer (see full README)
----
