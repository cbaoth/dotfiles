...
=== digital-detox-bedtime.sh - 😴 Digital Detox: Bedtime Enforcer
...
* The End: Once the timer hits zero, it executes a hard poweroff. No "one more video" allowed.
...
Move the script to your local bin and lock down the permissions so only root can touch it:

[source,shell]
----
sudo cp digital-detox-bedtime.sh /opt/bin/digital-detox-bedtime.sh
sudo chown root:root /opt/bin/digital-detox-bedtime.sh
sudo chmod 700 /opt/bin/digital-detox-bedtime.sh
----

NOTE: Open the script and make sure `+USER_NAME+` is set to your actual username (e.g., `+USER_NAME=yourname+`).
...
* Change the Time: Edit `+/etc/systemd/system/bedtime.timer+` and change the `+OnCalendar+` value.
* Change the Grace Period: Edit `+TIMEOUT+` in `+/opt/bin/digital-detox-bedtime.sh+`.

'''

=== nextcloud-maintenance.sh - 🔧 Nextcloud Maintenance Script
