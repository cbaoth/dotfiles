...
# Later on, when the file already exists, you can use the following instead,
# since `cp` may change file ownership:
...
Alternatively, the script can check if a USB flash drive (or any other block device) with a specific LABEL is present. To do so:

* Uncomment `+EMERGENCY_OVERRIDE_BLOCK_DEVICE_LABEL+` in `+/opt/bin/bedtime-shutdown.sh+` to your desired label (e.g., `BEDTIME-OVERRIDE`).
...
❓ *What about DST (Daylight Saving Time) changes?*
