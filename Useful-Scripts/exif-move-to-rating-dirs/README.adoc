...
The script maintains a cache of processed files (hash + modification time) to skip files that haven't changed. This significantly speeds up repeated runs.

* *Cache location*: `~/.cache/exif-move-to-rating-dirs/cache.tsv` (or `.cache.tsv.gz` if compressed)
* *Auto-compression*: Files larger than 1MB are automatically gzip-compressed
* *Cache validation*: Uses SHA256 hash + mtime for reliable change detection
