...
# --- EMERGENCY OVERRIDES (Uncomment to enable) ---
...
_exit_if_emergency_override() {
  ...
  if [[ -n "${EMERGENCY_OVERRIDE_MEDIA_LABEL:-}" ]]; then
    # We use lsblk to check all block device labels
    if lsblk -o LABEL -n | grep -Fqx "$EMERGENCY_OVERRIDE_MEDIA_LABEL"; then
      _notify_user "EMERGENCY OVERRIDE: Hardware key with label '$EMERGENCY_OVERRIDE_MEDIA_LABEL' detected. Skipping shutdown."
      exit 0
    fi
  fi
...
main() {
  ...
  systemctl poweroff --ignore-inhibitors --no-block
